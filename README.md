...
1. Create an OpsWorks Artifact Bucket in [S3](https://console.aws.amazon.com/s3/) and upload the zip file from https://github.com/awslabs/aws-codepipeline-s3-aws-codedeploy_linux/tree/master/dist. Be sure to enable versioning on the bucket for CodePipeline by right clicking on **Properties*1. for the bucket, select **Versioning*1. and click the **Enable Versioning*1. button.
1. Manually create an [OpsWorks](https://console.aws.amazon.com/opsworks/) stack using  w/ Chef 11.10. When creating an OpsWorks layer, use the **Static Web Server*1. Layer type. Use the **S3 Archive*1. repository type when creating the App in OpsWorks. Configure your OpsWorks stack with CodePipeline/CodeDeploy zip file deployed (from the above step). Once it's successfully deployed, make note of the *OpsWorks Stack ID1. and *OpsWorks App ID*.
1. Create an [SNS](https://console.aws.amazon.com/sns/) Topic. 
1. Select the newly-created SNS topic and click on the **Other topic actions*1. button, select **Edit topic Policy**. Go the **Advanced View*1. tab and copy the contents from `opsworks-sns-topic-policy.json`. Update the following variables: `REGION`, `AWSACCOUNTID`, `SNSNAME` and `OPSWORKSARTIFACTBUCKET`. Click the **Update policy*1. button.
1. With the newly-created SNS topic selected, click on the **Other topic actions*1. button and select **Edit topic delivery policy**. Then, click on the **Advanced View*1. tab and copy the source from: `opsworks-sns-topic-delivery-policy.json`. Click the **Update policy*1. button.
1. Create [Lambda IAM Role](https://console.aws.amazon.com/iam/home#roles). Use *AWS Lambda1. as the type. For the inline policy, copy the contents of `opsworks-lambda-role.json` and modify these values: `OPSWORKSARTIFACTBUCKET`, `OPSWORKSARN` and `SNSARN`. Click the **Validate policy*1. and **Apply policy*1. buttons.
...
1. Go the **Event Source*1. tab in Lambda and link it to the SNS Topic you previosuly created. Keep the **Enable Later*1. button selected.
1. Subscribe SNS to events for CodePipeline bucket in [S3](https://console.aws.amazon.com/s3/) by right clicking on the bucket, select **Properties**, then **Events**. Include `Put`, `Post`, `Copy` and `CompleteMultiPartUpload`. Make note of the *SNS ARN1. and the *SNS Event Subscription ARN1. by going to [SNS](https://console.aws.amazon.com/sns/) and selecting the Topic you created.
...
1. In [CodePipeline](https://console.aws.amazon.com/codepipeline/), add a new stage called `OpsWorks` (you can name it anything you’d like) and action in between the Source and Beta stages. When configuring the action, use the **Deploy*1. category and select the name of the custom action you just created fro the **Deployment Provider*1. drop down. Enter the three values for the parameters of the action. 
...
1. From the **Event Sources*1. tab of the [Lambda](https://console.aws.amazon.com/lambda/) function, click the `Disabled` state in the SNS source entry and click the **Enable*1. button.
1. Click **Release change*1. button on [CodePipeline](https://console.aws.amazon.com/codepipeline/)
